...
describe "Range", ->
  ...
  describe ".some", ->
    it "behaves as Array.some()", ->
      expect(new Liquid.Range(0, 4).some (v) -> v is 2).to.be.ok
      expect(new Liquid.Range(4, 0).some (v) -> v is 2).to.be.ok
      expect(new Liquid.Range(0, 4).some (v) -> v is 10).to.not.be.ok
      expect(new Liquid.Range(4, 0).some (v) -> v is 10).to.not.be.ok
