require "./_helper"
Liquid = require("../src/index")
Promise = require "bluebird"

asyncResult = (result) ->
  -> Promise.cast(result).delay(10)

describe "Futures", ->
  it "test_simple_variable", ->
    renderTest 'worked', '{{ test }}',
      test: asyncResult("worked")

  it "test_async_variable", ->
    Promise.all([
      renderTest 'worked', '{{ test.text }}',
        test: asyncResult({ text: "worked" })

      renderTest 'WORKED', '{{ test | upcase }}',
        test: asyncResult("worked")

      renderTest '1-2-3', '{{ array | join:minus }}',
        minus: asyncResult("-")
        array: [1, 2, 3]

      renderTest '1+2+3', '{{ array | join:minus | split:minus | join:plus }}',
        minus: asyncResult("-")
        plus: asyncResult("+")
        array: [1, 2, 3]

      renderTest 'YES', '{% if test %}YES{% else %}NO{% endif %}',
        test: asyncResult(true)

      renderTest 'NO', '{% if test %}YES{% else %}NO{% endif %}',
        test: asyncResult(false)

      renderTest 'NO', '{% unless test %}YES{% else %}NO{% endunless %}',
        test: asyncResult(true)

      renderTest 'YES', '{% unless test %}YES{% else %}NO{% endunless %}',
        test: asyncResult(false)

      renderTest 'Monkeys', '{% capture heading %}{{animal}}{% endcapture %}{{heading}}'
        animal: asyncResult("Monkeys")

      renderTest 'YES', '{% assign test = var %}{% if test %}YES{% else %}NO{% endif %}',
        var: true

      renderTest 'NO', '{% assign test = var %}{% if test %}YES{% else %}NO{% endif %}',
        var: false
    ])

  it "test_for_loop 1", ->
    products = ({ id: "item#{i}" } for i in [1, 2, 2])
    doc = "{% for product in products %}- {{ product.id }}\n{% endfor %}"
    renderTest "- item1\n- item2\n- item2\n", doc,
      products: asyncResult(products)

  it "test_for_loop 2", ->
    products = ({ id: "item#{i}" } for i in [1, 2, 2])
    doc = "{% for product in products %}{% ifchanged %}- {{ product.id }}\n{% endifchanged %}{% endfor %}"
    renderTest "- item1\n- item2\n", doc,
      products: asyncResult(products)

  it "test_for_loop_with_async_elements", ->
    doc = "{% for product in products %}- {{ product.id }}\n{% endfor %}"
    products = ({ id: asyncResult("item#{i}") } for i in [1..3])

    renderTest "- item1\n- item2\n- item3\n", doc,
      products: products

  ###
  TODO: This crashes due to too deep recursion.

  it "test_too_much_memory", ->
    doc = "{{ a"
    doc += ".a" while doc.length < (1024 * 1024)
    doc += ".b"
    doc += " }}"

    a = {}
    a.a = -> a
    a.b = -> "STOP"

    render "STOP", doc, a: a
